...
class ExplorerWorkspace(QtWidgets.QMainWindow):
    ...
    def _library_spectrum(self):
        ...
        if not self.act_centroid.isChecked():
            mz, intensity = centroid_spectrum(mz, intensity)
        precursor = None
        # the recalibration goes into the record's own provenance: a record
        # written from a corrected axis and one written from the instrument's
        # are different measurements of the same vial
